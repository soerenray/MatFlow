...
class Tests(unittest.TestCase):
    ...
    def testAuth(self):
        self.assertEqual(HTTPBasicAuth("airflow", "airflow"), self.testUserController.getAuth())

    # test getAllUsersAndDetails statuscode 200 method
    def testGetAllUsersAndDetails(self):
        self.assertEqual(requests.get("http://localhost:8080/api/v1/users", auth = self.testUserController.getAuth()).json(),self.testUserController.getAllUsersAndDetails)
    ...
    def testCreateUser(self):
        ...
        self.assertEqual(requests.get("http://localhost:8080/api/v1/users/testUsername", auth = self.testUserController.getAuth()).status_code, 200)
    

    # test deleteUser
    def testDeleteUser(self):
        self.testUserController.deleteUser(self.testUser)
        self.assertNotEqual(requests.get("http://localhost:8080/api/v1/users/testUsername", auth = self.testUserController.getAuth()).status_code, 200)
    ...
    def testCheckUsernames(self):
        ...
        self.testUserController.createUser("testUsername","testPassword","testPassword")
        self.assertEqual(requests.get("http://localhost:8080/api/v1/users/testUsername", auth = self.testUserController.getAuth()).json()["roles"][0]["name"],"Public")
        self.testUserController.overrideUser(overrideTestUser)
        print(self.testUserController.overrideUser(overrideTestUser))
        self.assertEqual(requests.get("http://localhost:8080/api/v1/users/testUsername", auth = self.testUserController.getAuth()).json()["roles"][0]["name"],"Viewer")
