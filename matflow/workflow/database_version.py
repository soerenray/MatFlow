...
from matflow.workflow.workflow_version import WorkflowVersion
...
class DatabaseVersion(WorkflowVersion):
    ...
    def __init__(
        self, version_number: VersionNumber, note: str, changed_config_files: Path
    ):
    ...
    def get_frontend_version(
        self, old_config_files: List[Tuple[str, Path]]
    ) -> FrontendVersion:
        ...
        for name, old_path in old_config_files:
            ...
            for change in changes:
                parameter_changes.append(
                    ParameterChange(change[0], change[1], change[2], change[3], name)
                )
        return FrontendVersion(
            self.get_version_number(), self.get_note(), parameter_changes
        )
