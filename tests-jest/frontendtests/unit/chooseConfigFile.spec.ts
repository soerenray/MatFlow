...
describe('ChooseConfigFile.vue', () => {
  beforeEach(() => {
    wrapper = shallowMount(ChooseConfigFileView, { });
  });

  // (method) colorForConfigFileName: (updatedConfigFiles: ConfigFile[], selectedConfigFileName:
  // string, configFileName: string) => string
  it('colorForConfigFileName', () => {
    ...
    // @ts-ignore
    expect(wrapper.vm.colorForConfigFileName(
      updatedConfigFiles,
      configFileName1,
...
// (method) isConfigFileNameInUpdatedConfigFiles(updatedConfigFiles: ConfigFile[],
// configFileName: string): boolean
it('isConfigFileNameInUpdatedConfigFiles', () => {
  ...
  // @ts-ignore
  expect(wrapper.vm.isConfigFileNameInUpdatedConfigFiles(
    updatedConfigFiles,
    configFileName1,
  )).toBe(false);
...
// (method) changeAllKeyValuePairs(configFileName: string, newKeyValuePairs:
...
it('changeAllKeyValuePairs', () => {
  ...
  expect(keyValuePair2).toEqual(keyValuePair1);
});

// (method) updateKeyValuePairs(oldKetValuePairs: Array<[string, string]>,
// newKeyValuePairs: Array<[string, string]>): void
...
// (method) getConfigFileFromUpdatedConfigFiles(updatedConfigFiles: ConfigFile[],
...
it('getConfigFileFromUpdatedConfigFiles', () => {
  ...
  // @ts-ignore
  expect(wrapper.vm.getConfigFileFromUpdatedConfigFiles(
    updatedConfigFiles,
    conf1Name,
  )).toBe(conf1File);
  ...
  // @ts-ignore
  expect(() => {
    wrapper.vm.getConfigFileFromUpdatedConfigFiles(
      updatedConfigFiles,
      conf2Name,
    );
  }).toThrow();

  // Checks if the name conf2Name is in a configFile of updatedConfigFiles and
  // throws an error with message "'There is no configFile with name ' + conf2Name"
  // @ts-ignore
  expect(() => {
    wrapper.vm.getConfigFileFromUpdatedConfigFiles(
      updatedConfigFiles,
  ...
  }).toThrow(`There is no configFile with name ${conf2Name}`);
});

// (method) setSelectedWorkflowInstanceNameAndResetConfigFileNameAndUpdatedConfigFiles
// (selectedWorkflowInstanceName: string): void
