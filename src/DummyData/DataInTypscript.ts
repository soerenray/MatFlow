// Just a dummy file, should not ship in production
/* eslint-disable */
...
const workflowInstancesNameAndConfigFilesName: Array<[string, string[]]> = [
  ['workflowInstance1', [
    'conf1',
    'conf2',
  ]],
...
function pullServers2() {
  ...
  return [
    new Server(
      JSON.parse(JSON.stringify(server.serverAddress)),
      JSON.parse(JSON.stringify(server.serverStatus)),
      JSON.parse(JSON.stringify(server.containerLimit)),
      JSON.parse(JSON.stringify(server.selectedForExecution)),
      JSON.parse(JSON.stringify(server.serverName)),
      JSON.parse(JSON.stringify(server.serverResources)),
    )];
}
...
function deepCopyConfigFile(configFile: ConfigFile): ConfigFile {
  return new ConfigFile(
    JSON.parse(JSON.stringify(configFile.configFileName)),
    JSON.parse(JSON.stringify(configFile.keyValuePairs)),
  );
}
...
function updateUser(user: User) {
  const userIndex = users.findIndex((userInUsers: User) => user.userName === userInUsers.userName);
  if (userIndex == -1) {
...
const versions: VersionsObject = {
  ...
  workflowInstance2: [new Version('2.1', 'changed name of key3', [['key3: Foo bar', 'key1: Foo bar']]), new Version('2.2', 'fixed typo in key-value', [['key1: xy', 'key1: xy'], ['key2: 42', 'key2: 420'],
  ])],
};
...
export {
  templateNames, workflowInstancesNameAndConfigFilesName, getWfConf, setWfConf, versions, users, deleteUser, updateUser, pullServers2, pushServer,
};
