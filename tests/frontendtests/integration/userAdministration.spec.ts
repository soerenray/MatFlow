...
BackendServerCommunicatorSimulation.prototype.pullUsers = function () {
    return this.users
}
...
describe('UserAdministration', () => {
    ...
    it("changes made to both users should be applied", () => {
        cy.get("[data-cy=userPriviliges]").eq(0).click().get('.v-overlay-container .v-list:visible').contains('activated').click()
        cy.get("[data-cy=userPriviliges]").eq(1).click().get('.v-overlay-container .v-list:visible').contains('suspended').click()
        cy.get("[data-cy=userStatus]").eq(0).click().get('.v-overlay-container .v-list:visible').contains('visitor').click()
        cy.get("[data-cy=userStatus]").eq(1).click().get('.v-overlay-container .v-list:visible').contains('developer').click()
