...
BackendServerCommunicatorSimulation.prototype.pullConfigFileWithConfigFileNameWithWorkflowInstanceName = function () {
...
describe('ChooseConfigFile', () => {
  ...
  it('keys and values preserve their entries when it is switched between the config files of a workflow instances', () => {
    ...
    cy.get("[data-cy=keyEntry] .v-field__input").eq(0).click().clear().type('key30')
    cy.get("[data-cy=valueEntry] .v-field__input").eq(0).click().clear().type('val30')
    cy.then(() => {
