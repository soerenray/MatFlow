...
BackendServerCommunicatorSimulation.prototype.pullVersionsWithWorkflowInstanceName = function () {
  return this.versions
}
BackendServerCommunicatorSimulation.prototype.pullWorkflowInstancesNameAndConfigFilesName = function () {
  return this.workflowInstancesNameAndConfigFilesName
}
...
describe("VersionControl table content check", () => {
  ...
  it("The tablebody has the correct (versionNumber, version note)-pairs in the correct order after clicked on 'workflowInstance1'", () => {
    backendServerCommunicatorObject.versions = [workflowInstance1Version1, workflowInstance1Version2]
    cy.get("[data-cy=workflowInstancesName] .v-col").eq(0).click()
    cy.get("[data-cy=tableBody] > tr").eq(0).children().eq(0).should('have.text', '1.1')
    cy.get("[data-cy=tableBody] > tr").eq(0).children().eq(1).should('have.text', 'changed value of key1')
    cy.get("[data-cy=tableBody] > tr").eq(1).children().eq(0).should('have.text', '1.1.1')
    cy.get("[data-cy=tableBody] > tr").eq(1).children().eq(1).should('have.text', 'reverted previous change')
  })
  ...
  it("The tablebody has the correct (versionNumber, version note)-pairs in the correct order after clicked on 'workflowInstance1' and then 'workflowInstance2'", () => {
    ...
    cy.get("[data-cy=workflowInstancesName] .v-col").eq(1).click()
    cy.get("[data-cy=tableBody] > tr").eq(0).children().eq(0).should('have.text', '2.1')
    cy.get("[data-cy=tableBody] > tr").eq(0).children().eq(1).should('have.text', 'changed name of key3')
    cy.get("[data-cy=tableBody] > tr").eq(1).children().eq(0).should('have.text', '2.2')
    cy.get("[data-cy=tableBody] > tr").eq(1).children().eq(1).should('have.text', 'fixed typo in key-value')
  })
  ...
  it("The changed parameters of version '1.1' of 'workflowInstance1' should be displayed correctly in the correct order", () => {
    ...
    cy.get("[data-cy=workflowInstancesName] .v-col").eq(0).click()
    cy.get("[data-cy=fileButton]").eq(0).click()
    cy.get("[data-cy=keyValuePair] [data-cy=tableBody] > tr").eq(0).children().eq(0).should('have.text', 'key1: Ipsom lorum')
    cy.get("[data-cy=keyValuePair] [data-cy=tableBody] > tr").eq(1).children().eq(0).should('have.text', 'key1: xy')
    cy.get("[data-cy=keyValuePair] [data-cy=tableBody] > tr").eq(2).children().eq(0).should('have.text', 'key3: 5.0 5.0')
    cy.get("[data-cy=keyValuePair] [data-cy=tableBody] > tr").eq(0).children().eq(1).should('have.text', "key1: lorem ipsum")
    cy.get("[data-cy=keyValuePair] [data-cy=tableBody] > tr").eq(1).children().eq(1).should('have.text',  "key2: xy")
    cy.get("[data-cy=keyValuePair] [data-cy=tableBody] > tr").eq(2).children().eq(1).should('have.text', "key3: 'text'")
  })
  ...
  it("The changed parameters of version '2.2' of 'workflowInstance2' should be displayed correctly in the correct order", () => {
    ...
    cy.get("[data-cy=keyValuePair] [data-cy=tableBody] > tr").eq(0).children().eq(0).should('have.text', "key1: xy")
    cy.get("[data-cy=keyValuePair] [data-cy=tableBody] > tr").eq(1).children().eq(0).should('have.text', "key2: 42")
    cy.get("[data-cy=keyValuePair] [data-cy=tableBody] > tr").eq(0).children().eq(1).should('have.text', "key1: xy")
    cy.get("[data-cy=keyValuePair] [data-cy=tableBody] > tr").eq(1).children().eq(1).should('have.text',  "key2: 420")
  })
})