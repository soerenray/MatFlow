...
class IntegrationTest(unittest.TestCase):
    ...
    def test_create_version(cls):
        ...
        got = json.loads(cls.app.post("create_version_of_wf_instance", json=send_off).get_data())
    ...
    def test_set_user_details(self):
        payload = json.dumps({keys.user_name: "first_user", keys.user_status_name: True,
                              keys.user_privilege_name: "Admin", keys.password_name: "airflow"})
    ...
    def test_delete_user(self):
        payload = json.dumps({keys.user_name: "first_user", keys.password_name: "default",
                              keys.user_privilege_name: "Public", keys.user_status_name: True})
        ...
        self.assertNotIn({keys.user_name: "first_user",keys.user_status_name: True,
                                          keys.user_privilege_name: "Public"}, all[keys.all_users])
    ...
    def test_replace_version(self):
        ...
        payload = {keys.workflow_instance_name: "test_instance", keys.version_number_name: "Florian bitte"
                                                                                           "einfuegen"}
        got = json.loads(self.__class__.app.put("replace_active_wf_instance_version",
                                                json = json.dumps(payload)).get_data())
        self.assertEqual(got, {keys.status_code_name: 607})
...
class SetUpTester(unittest.TestCase):
    ...
    def test_create_wf_instance(self):
        ...
        input_wf_dict = {keys.workflow_instance_name: wf_name,
                             ...
                             keys.config_files: [{keys.file_key: encoded_config.decode("utf-8"),
                                                  keys.config_file_name: "test1.conf"}]}
        send_off = json.dumps(input_wf_dict)
        got = json.loads(self.app.post("create_workflow_instance", json=send_off).get_data())
    ...
    @unittest.skip("Florian und Lukas, die Version ist nicht in der db, wf instanz ist aber schon gemerkt")
    def test_create_version(self):
        ...
        got = json.loads(self.app.post("create_version_of_wf_instance", json=send_off).get_data())
        self.assertEqual(got, {keys.status_code_name: 607})
